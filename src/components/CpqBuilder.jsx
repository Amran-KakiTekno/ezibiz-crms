...
export default function CpqBuilder({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-7 rounded-2xl bg-white dark:bg-zinc-950/80 border border-slate-200 dark:border-white/[0.08] shadow-sm p-6 space-y-6">
        <div>
          ...
          <h3 className="text-base font-bold text-slate-900 dark:text-zinc-100">Dynamic Scope & Pricing Configurator</h3>
              ...
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer ${
                  baseTier === t.id 
                    ...
                    : 'bg-slate-50 dark:bg-black/60 border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-zinc-400 hover:border-slate-300 dark:hover:border-white/[0.16]'
                }`}
              >
                <div className="flex items-center justify-between">
                  <p className="font-semibold text-slate-900 dark:text-zinc-100">{t.title}</p>
                  {baseTier === t.id && (
                ...
                <p className="text-[10px] text-slate-500 dark:text-zinc-500 mt-1 leading-tight">{t.desc}</p>
              </button>
            ))}
          </div>
        </div>
              ...
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                  depositPct === pct 
                    ...
                    : 'bg-slate-100 dark:bg-black border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                {pct}% Preset
              </button>
      ...
      <div className="lg:col-span-5 rounded-2xl bg-white dark:bg-zinc-950 border border-slate-200 dark:border-white/[0.08] shadow-sm p-6 flex flex-col justify-between space-y-6 relative overflow-hidden">
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/[0.08] pb-4">
            <div>
              ...
              <h4 className="text-base font-bold text-slate-900 dark:text-zinc-100 mt-0.5">Interactive Order #PR-402</h4>
            </div>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between text-slate-500 dark:text-zinc-400">
              ...
              <span className="text-slate-900 dark:text-zinc-100 font-mono tabular-nums font-medium">
            ...
            {addOns.florals && (
              <div className="flex justify-between text-slate-500 dark:text-zinc-400">
                ...
                <span className="text-slate-800 dark:text-zinc-200 font-mono tabular-nums">+{formatCurrency(150)}</span>
              </div>
            )}
            {addOns.glutenFree && (
              <div className="flex justify-between text-slate-500 dark:text-zinc-400">
                <span>Gluten-Free Blend:</span>
                <span className="text-slate-800 dark:text-zinc-200 font-mono tabular-nums">+{formatCurrency(85)}</span>
              </div>
            )}
            {addOns.rushSetup && (
              <div className="flex justify-between text-slate-500 dark:text-zinc-400">
                <span>Priority Rush 48h:</span>
                <span className="text-slate-800 dark:text-zinc-200 font-mono tabular-nums">+{formatCurrency(180)}</span>
              </div>
            )}
            {addOns.monogram && (
              <div className="flex justify-between text-slate-500 dark:text-zinc-400">
                <span>Custom Monogram:</span>
                <span className="text-slate-800 dark:text-zinc-200 font-mono tabular-nums">+{formatCurrency(60)}</span>
              </div>
            )}

            <div className="pt-3 border-t border-slate-200 dark:border-white/[0.08] flex justify-between text-sm font-semibold">
              <span className="text-slate-700 dark:text-zinc-300">Total Project Scope:</span>
              <span className="text-slate-900 dark:text-zinc-100 font-mono tabular-nums text-base">{formatCurrency(totalAmount)}</span>
            </div>
          </div>
        ...
        <div className="space-y-2 pt-2">
          ...
          <button
            ...
            className="w-full py-2 rounded-xl bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-white/[0.08] text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white text-xs font-medium flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-sm"
          >
